...
export function createToolsDock(app, theme) {
  ...
  ui.downloadBtn.addEventListener('click', event => {
    ...
    function serializeStandaloneDocument() {
      ...
      // Remove older copies of this bootstrap when an exported file is exported
      // again, then install one early classic script. The one-time local reload
      // works around Chromium's first-open file:// console warning; the marker is
      // removed from the visible URL immediately after the clean reload.
      clone.querySelectorAll?.('script[data-automind-standalone-bootstrap="1"]')
        .forEach(element => element.remove());
      const bootstrap = document.createElement('script');
      bootstrap.dataset.automindStandaloneBootstrap = '1';
...
(function () {
  ...
  try {
    ...
    try { history.replaceState(null, document.title, url.href); } catch (_) {}
  } catch (_) {}
})();`;
      const cloneHead = clone.querySelector?.('head') || clone;
      cloneHead.insertBefore(bootstrap, cloneHead.firstChild || null);
