...
export function createToolsDock(app, theme) {
  ...
  // Download the standalone viewer HTML. Prefer the original source (srcdoc,
  // blob/data URL or an actual .html URL); serialize the document only as a fallback.
  ui.downloadBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (!source) {
        ...
        const looksLikeHtml = /\.html?(?:[?#].*)?$/i.test(href);
        if (href && (protocol === 'blob:' || protocol === 'data:' || looksLikeHtml)) {
          try {
      ...
      if (!source) {
        // Fallback: clone the current document, but remove runtime UI/canvas so the
        // embedded startup scripts recreate exactly one clean viewer when reopened.
        ...
        const doctype = document.doctype
          ...
          : '<!DOCTYPE html>\n';
        source = doctype + clone.documentElement.outerHTML;
      }
