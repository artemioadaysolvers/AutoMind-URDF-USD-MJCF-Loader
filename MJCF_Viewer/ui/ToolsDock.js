...
` +
  ...
  function disposeMaterialSet(value) {
    for (const material of materialArray(value)) {
      try { material?.dispose?.(); } catch (_) {}
    }
  }

  function purgeLegacySectionCache(mesh) {
    const ud = mesh?.userData;
    if (!ud) return;
    const original = ud.__clipOriginalMaterial;
    const cloned = ud.__clipClonedMaterial;
    if (!original && !cloned) return;

    // Migration from BUILD266 and older: restore a cached original only when the
    // currently active material is exactly the old section clone. Never touch an
    // inspection material or a newer authored material.
    if (original && sameMaterialReference(mesh.material, cloned)) mesh.material = original;
    if (cloned && !sameMaterialReference(mesh.material, cloned) && !sameMaterialReference(original, cloned)) {
      disposeMaterialSet(cloned);
    }
    delete ud.__clipOriginalMaterial;
    delete ud.__clipClonedMaterial;
  }
  ...
  function applyRobotOnlyClipping(plane) {
    ...
    traverseRobotMeshes((mesh) => {
      purgeLegacySectionCache(mesh);
      for (const material of materialArray(mesh.material)) setMaterialClipping(material, plane);
    });
  }
  ...
  return {
    ...
    refreshRenderMode: () => setRenderMode(renderModeSel.value || 'Solid'),
