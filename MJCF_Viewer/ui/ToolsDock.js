...
export function createToolsDock(app, theme) {
  ...
  async function fetchStandaloneThreeSource() {
    const candidates = [];
    for (const script of Array.from(document.scripts || [])) {
      const src = String(script?.src || '');
      if (/(?:^|\/)three(?:\.min)?\.js(?:[?#]|$)/i.test(src)) candidates.push(src);
    }
    candidates.push(
    ...
    for (const url of [...new Set(candidates.filter(Boolean))]) {
      try {
        ...
        if (!source || !/WebGLRenderer/.test(source) || !/Vector3/.test(source)) continue;
  ...
  ui.downloadBtn.addEventListener('click', async event => {
    ...
    const standaloneThreeSource = await fetchStandaloneThreeSource();

    function serializeStandaloneDocument(embeddedThreeSource = '') {
      ...
      // Guard dynamic runtime code as well as static markup. A whitespace-only
      // URL on file:// resolves to the document itself and Chromium prints an
      // unsafe unique-origin warning. This guard removes such attributes before
      // the browser is asked to resolve them.
      const fileSafeGuard = document.createElement('script');
      fileSafeGuard.dataset.automindFileSafeGuard = '1';
      fileSafeGuard.textContent = `(function(){
` +
        `  'use strict';
` +
        `  const urlNames=new Set(['src','href','poster','data','action','formaction','cite','manifest']);
` +
        `  const resourceTags=new Set(['SCRIPT','IMG','IFRAME','FRAME','SOURCE','AUDIO','VIDEO','TRACK','EMBED','OBJECT','LINK','INPUT']);
` +
        `  const currentFile=location.protocol==='file:' ? location.href.split('#')[0].split('?')[0] : '';
` +
        `  function resolvedRequest(value){
` +
        `    let raw='';
` +
        `    try{ raw=value instanceof Request ? value.url : String(value==null?'':value).trim(); }catch(_){ raw=''; }
` +
        `    if(!raw) return {blocked:true,raw:'',resolved:currentFile};
` +
        `    try{
` +
...
` +
        `      return {blocked:!!currentFile && normalized===currentFile,raw,resolved:absolute};
` +
        `    }catch(_){ return {blocked:false,raw,resolved:raw}; }
` +
        `  }
` +
...
` +
        `      const check=resolvedRequest(input);
` +
        `      if(check.blocked){
` +
        `        try{ console.debug('[AutoMind standalone] blocked self file fetch',check.raw); }catch(_){}
` +
...
` +
        `      const check=resolvedRequest(url);
` +
        `      if(check.blocked){
` +
        `        try{ console.debug('[AutoMind standalone] blocked self file XHR',check.raw); }catch(_){}
` +
        `        return nativeOpen.call(this,method,'data:application/octet-stream;base64,',...rest);
` +
        `      }
` +
...
` +
        `  }catch(_){}
` +
        `  function isBlocked(element,name,value){
` +
        `    name=String(name||'').toLowerCase();
` +
        `    if(!urlNames.has(name)) return false;
` +
        `    const raw=String(value==null?'':value).trim();
` +
        `    if(!raw) return true;
` +
        `    if(!currentFile || !resourceTags.has(String(element?.tagName||'').toUpperCase())) return false;
` +
        `    if(raw[0]==='#') return false;
` +
        `    try{
` +
        `      const resolved=new URL(raw,document.baseURI).href.split('#')[0].split('?')[0];
` +
...
` +
        `      if(isBlocked(element,name,element.getAttribute(name))){ try{element.removeAttribute(name);}catch(_){} }
` +
        `    }
` +
        `  }
...
` +
        `    if(isBlocked(this,name,value)){ try{this.removeAttribute(name);}catch(_){} return; }
` +
...
` +
        `    const local=String(name||'').split(':').pop();
` +
        `    if(isBlocked(this,local,value)){ try{this.removeAttributeNS(namespace,local);}catch(_){} return; }
` +
        `    return originalNS.call(this,namespace,name,value);
` +
        `  };
` +
        `  const guard=function(proto,name){
` +
        `    try{
` +
...
` +
        `      Object.defineProperty(proto,name,{configurable:d.configurable,enumerable:d.enumerable,get:d.get,set:function(value){
` +
        `        if(isBlocked(this,name,value)){ try{this.removeAttribute(name);}catch(_){} return; }
` +
...
` +
        `  };
` +
        `  guard(HTMLImageElement.prototype,'src');
` +
        `  guard(HTMLMediaElement.prototype,'src');
` +
        `  guard(HTMLScriptElement.prototype,'src');
` +
        `  guard(HTMLSourceElement.prototype,'src');
` +
        `  guard(HTMLLinkElement.prototype,'href');
` +
        `  if(globalThis.HTMLIFrameElement) guard(HTMLIFrameElement.prototype,'src');
` +
        `  if(globalThis.HTMLFrameElement) guard(HTMLFrameElement.prototype,'src');
` +
        `  if(globalThis.HTMLObjectElement) guard(HTMLObjectElement.prototype,'data');
` +
        `  if(globalThis.HTMLEmbedElement) guard(HTMLEmbedElement.prototype,'src');
` +
        `  if(globalThis.HTMLTrackElement) guard(HTMLTrackElement.prototype,'src');
` +
        `  if(globalThis.HTMLInputElement) guard(HTMLInputElement.prototype,'src');
` +
        `  const scan=function(root){
` +
        `    sanitize(root);
` +
        `    try{ root?.querySelectorAll?.('[src],[href],[poster],[data],[action],[formaction],[cite],[manifest]').forEach(sanitize); }catch(_){}
` +
        `  };
` +
...
` +
        `      if(record.type==='attributes') sanitize(record.target);
` +
...
` +
        `  });
` +
        `  observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:[...urlNames]});
` +
        `})();`;
      cloneHead.insertBefore(fileSafeGuard, cloneHead.firstChild || null);
      ...
      // Make the downloaded file deterministic: when possible, embed the exact
      // Three.js source directly in the document. This removes the CDN race that
      // previously produced `THREE is not defined` / `undefined.Vector3` on file://.
      ...
      if (embeddedThreeSource) {
        for (const script of [...threeScripts, ...orbitScripts]) script.remove();
        const embeddedThree = document.createElement('script');
        embeddedThree.dataset.automindEmbeddedThree = '1';
        embeddedThree.textContent = String(embeddedThreeSource)
          .replace(/<\/script/gi, '<\\/script') +
          '\n;globalThis.THREE = globalThis.THREE || (typeof THREE !== "undefined" ? THREE : undefined);';
        cloneHead.insertBefore(embeddedThree, cloneHead.firstChild || null);
      } else {
        // Network fallback: parser-blocking scripts still precede every module.
      ...
      // Every inline module gets a local THREE binding only after the namespace is
      // verified. This also fixes modules opened through file://, where relying on
      // an implicit identifier created by a classic script is browser-dependent.
      clone.querySelectorAll?.('script[type="module"]:not([src])').forEach(script => {
        let code = String(script.textContent || '');
        code = code.replace(/\/\* __AUTOMIND_WAIT_FOR_THREE__ \*\/[\s\S]*?const THREE = globalThis\.THREE;\s*/m, '');
        code = code.replace(/\n\s*const THREE = globalThis\.THREE;(?=\n)/g, '');
        script.textContent = `/* __AUTOMIND_WAIT_FOR_THREE__ */\n` +
          ...
          `if (!(globalThis.THREE && globalThis.THREE.Vector3)) throw new Error('Three.js could not be initialized in the standalone AutoMind HTML.');\n` +
          `const THREE = globalThis.THREE;\n` + code;
      });
      ...
      clone.querySelectorAll?.('[src], [href], [poster], [data], [action], [formaction], [cite], [manifest]').forEach(element => {
        const resourceTags = new Set(['SCRIPT', 'IMG', 'IFRAME', 'FRAME', 'SOURCE', 'AUDIO', 'VIDEO', 'TRACK', 'EMBED', 'OBJECT', 'LINK', 'INPUT']);
        const currentFile = location.protocol === 'file:' ? location.href.split('#')[0].split('?')[0] : '';
        for (const attribute of ['src', 'href', 'poster', 'data', 'action', 'formaction', 'cite', 'manifest']) {
          if (!element.hasAttribute?.(attribute)) continue;
          const value = String(element.getAttribute(attribute) || '').trim();
          let remove = !value;
          if (!remove && currentFile && resourceTags.has(String(element.tagName || '').toUpperCase()) && value[0] !== '#') {
            try {
              const resolved = new URL(value, document.baseURI).href.split('#')[0].split('?')[0];
              remove = resolved === currentFile;
            } catch (_) {}
          }
      ...
      const doctype = document.doctype
        ...
        : '<!DOCTYPE html>\n';
      let serialized = doctype + clone.outerHTML;
      // Chromium resolves empty resource URLs against the current file. In a
      // file:// document that produces the distracting unique-origin warning.
      // Sanitize both attributes and CSS forms after serialization so no parser
      // normalization can reintroduce an empty URL.
      serialized = serialized
        .replace(/\s(?:src|href|poster|data|action|formaction|cite|manifest)=(['"])\s*\1/gi, '')
        .replace(/url\(\s*(['"]?)\s*\1\s*\)/gi, 'none')
        .replace(/<source\b([^>]*?)\s+src=(['"])\s*\2([^>]*)>/gi, '<source$1$3>');
      return serialized;
    }

    try {
      const source = serializeStandaloneDocument(standaloneThreeSource);
      // Descarga directamente el documento HTML independiente. No se envuelve
      // en un launcher que reemplace la página por una URL blob:null al abrirlo.
      // De este modo la barra conserva file:///.../<nombre>.html.
  ...
  function setRenderMode(mode) {
    const normalizedMode = ['Solid', 'Wireframe', 'X-Ray', 'Ghost'].includes(String(mode)) ? String(mode) : 'Solid';
    if (renderModeSel.value !== normalizedMode) renderModeSel.value = normalizedMode;
    mode = normalizedMode;
  ...
  function isInspectionMaterialSet(value) {
    const materials = materialArray(value);
    return materials.length > 0 && materials.every(material => !!material?.userData?.__automindInspectionMaterial);
  }

  function sameMaterialReference(a, b) {
    if (a === b) return true;
    if (!Array.isArray(a) || !Array.isArray(b) || a.length !== b.length) return false;
    return a.every((value, index) => value === b[index]);
  }

  function disposeClippingClone(value) {
  ...
  function clearStaleClippingCache(mesh) {
    if (!mesh?.userData) return;
    const original = mesh.userData.__clipOriginalMaterial;
    const cloned = mesh.userData.__clipClonedMaterial;
    if (!original && !cloned) return;

    // Component inspection temporarily replaces mesh.material. If Section is
    // enabled while that temporary material is active, the old cache must not
    // later restore/dispose the inspection material as if it were the CAD one.
    if (isInspectionMaterialSet(mesh.material)) return;
    if (!sameMaterialReference(mesh.material, original) && !sameMaterialReference(mesh.material, cloned)) {
      disposeClippingClone(cloned);
      delete mesh.userData.__clipOriginalMaterial;
      delete mesh.userData.__clipClonedMaterial;
    }
  }
  ...
  function ensureClippableMaterials(mesh) {
    ...
    mesh.userData ??= {};
    clearStaleClippingCache(mesh);
    if (mesh.userData.__clipOriginalMaterial) return;
  ...
  function applyRobotOnlyClipping(plane) {
    ...
    traverseRobotMeshes((mesh) => {
      // Component-detail materials are already private and temporary. Clip them
      // directly, without replacing the persistent CAD section cache. This is
      // the critical case when Enable Section is switched on after selecting a
      // component and before pressing Back.
      if (!isInspectionMaterialSet(mesh.material)) {
  ...
  function clearRobotOnlyClipping() {
    ...
    traverseRobotMeshes((mesh) => {
      if (isInspectionMaterialSet(mesh.material)) {
        // Section can be disabled while the component sheet is still open.
        // Clear only the temporary detail materials now. Keep any canonical CAD
        // cache until endComponentInspection restores its recorded material;
        // refreshSection() will then perform the normal final restoration.
        for (const material of materialArray(mesh.material)) setMaterialClipping(material, null);
        const cachedClone = mesh.userData?.__clipClonedMaterial;
        for (const material of materialArray(cachedClone)) setMaterialClipping(material, null);
        return;
      }
  ...
  return {
    open: openDock, close: closeDock, set, destroy,
    setRenderMode: mode => setRenderMode(mode),
    getRenderMode: () => renderModeSel.value || 'Solid',
    refreshSection: () => updateSectionPlane(),
    getSectionState: () => ({
      enabled: !!secEnabled,
      showPlane: !!secPlaneVisible,
      axis: secAxis,
      distance: Number(secDist?.value || 0),
