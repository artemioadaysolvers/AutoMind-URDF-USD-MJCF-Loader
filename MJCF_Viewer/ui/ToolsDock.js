...
export function createToolsDock(app, theme) {
  ...
  ui.downloadBtn.addEventListener('click', event => {
    ...
    function serializeStandaloneDocument() {
      ...
      // The host bootstrap is a module script. Do not rely on a bare THREE
      // identifier being created from window.THREE under file://. Give the
      // texture safety function an explicit local binding in the downloaded
      // document so it cannot emit a recoverable ReferenceError on startup.
      clone.querySelectorAll?.('script:not([src])').forEach(script => {
        const code = String(script.textContent || '');
        const signature = 'function installTextureSafetyPatch() {';
        if (!code.includes(signature) || code.includes('const THREE = globalThis.THREE;')) return;
