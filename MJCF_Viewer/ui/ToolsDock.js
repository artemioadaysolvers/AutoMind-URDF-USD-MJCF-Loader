...
export function createToolsDock(app, theme) {
  ...
  // Download the standalone viewer HTML without ever fetching a file:// URL.
  // Local files are unique origins, so they must be serialized into a Blob directly.
  ui.downloadBtn.addEventListener('click', async () => {
    ...
    const serializeCurrentDocument = () => {
      try { app.renderer.domElement.dataset.automindRuntimeCanvas = '1'; } catch (_) {}
      const clone = document.cloneNode(true);
      clone.querySelectorAll?.('[data-automind-runtime-ui="1"], [data-automind-runtime-canvas="1"]')
        .forEach(el => el.remove());
      try { delete app.renderer.domElement.dataset.automindRuntimeCanvas; } catch (_) {}
    ...
    try {
      ...
      // srcdoc already contains the complete standalone source and does not need
      // a network request or access to the frame's file origin.
      try {
        const srcdoc = window.frameElement?.getAttribute?.('srcdoc') || '';
        if (srcdoc.trim()) source = srcdoc;
      } catch (_) {}

      const protocol = String(window.location?.protocol || '').toLowerCase();
      const href = String(window.location?.href || '');

      // Never call fetch() for file://. Chromium treats each local file as a
      // different security origin and logs/blocks exactly the error reported.
      if (!source && ['http:', 'https:', 'blob:', 'data:'].includes(protocol) && href) {
        try {
      ...
      if (!source) source = serializeCurrentDocument();
      downloadBlob(source);
    } catch (error) {
      console.error('[Download HTML] failed:', error);
      try {
        downloadBlob(serializeCurrentDocument());
      } catch (_) {
  ...
  bIsoEl.addEventListener('click', () => { const v = to('iso'); tweenOrbits(app.camera, app.controls, v.pos, v.target, 750, v.projectionTarget); });
  bTopEl.addEventListener('click', () => { const v = to('top'); tweenOrbits(app.camera, app.controls, v.pos, v.target, 750, v.projectionTarget); });
  bFrontEl.addEventListener('click', () => { const v = to('front'); tweenOrbits(app.camera, app.controls, v.pos, v.target, 750, v.projectionTarget); });
  bRightEl.addEventListener('click', () => { const v = to('right'); tweenOrbits(app.camera, app.controls, v.pos, v.target, 750, v.projectionTarget); });
