...
export function createComponentsPanel(app, theme) {
  ...
  function buildDetail(record) {
    ...
    ui.detailView.appendChild(relationshipChipsRow('Parent body', [parent?.node?.name || parent?.linkName || 'Root'], 'parent'));
    ui.detailView.appendChild(relationshipChipsRow('Child bodies', children.map(c => c.name || c.linkName).filter(Boolean), 'child'));
