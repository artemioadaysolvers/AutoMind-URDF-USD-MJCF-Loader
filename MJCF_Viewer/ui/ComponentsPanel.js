// AutoMind MJCF body-tree Components panel — BUILD266 state-safe Tools/Components transaction.
// BUILD248:
...
export function createComponentsPanel(app, theme) {
  ...
  Object.assign(ui.backBtn.style, {
    display: 'none', width: '31px', height: '31px', borderRadius: '10px', border: `1px solid ${theme.stroke}`,
    background: theme.bgPanel, color: theme.text, fontWeight: '900', fontSize: '17px', cursor: 'pointer',
    alignItems: 'center', justifyContent: 'center', padding: '0',
    boxShadow: theme.shadow, transition: 'transform .12s ease, box-shadow .12s ease, background-color .12s ease, border-color .12s ease'
  });
  ...
  Object.assign(ui.downloadSheetBtn.style, {
    display: 'none', padding: '6px 10px', borderRadius: '10px', border: `1px solid ${theme.stroke}`,
    background: theme.bgPanel, color: theme.text, fontWeight: '800', cursor: 'pointer', fontSize: '11px',
    boxShadow: theme.shadow, transition: 'transform .12s ease, box-shadow .12s ease, background-color .12s ease, border-color .12s ease'
  });
  ...
  [ui.btn, ui.backBtn, ui.downloadSheetBtn].forEach(button => { button.type = 'button'; });

  ui.title.textContent = 'Model hierarchy';
  ui.downloadSheetBtn.textContent = 'Download Technical Report';
  ui.downloadSheetBtn.title = 'Download Technical Report';
  ...
  ui.headerActions.append(ui.downloadSheetBtn);
  ...
  function installComponentsButtonHover(button, hoverTransform = 'translateY(-2px)') {
    button.addEventListener('mouseenter', () => {
      button.style.transform = hoverTransform;
      button.style.background = theme.tealFaint || '#ecfeff';
      button.style.borderColor = theme.tealSoft || theme.teal || '#0ea5a6';
      button.style.boxShadow = '0 10px 26px rgba(14,165,166,.20)';
    });
    button.addEventListener('mouseleave', () => {
      button.style.transform = 'none';
      button.style.background = theme.bgPanel;
      button.style.borderColor = theme.stroke;
      button.style.boxShadow = theme.shadow;
    });
  }
  installComponentsButtonHover(ui.backBtn, 'translateX(-1px)');
  installComponentsButtonHover(ui.downloadSheetBtn);
  ...
  function setPanelOpen(isOpen) {
    const closingDetail = !isOpen && viewMode === 'detail';

    // Closing Components from a technical sheet is semantically identical to
    // Back. Never leave temporary inspection materials alive behind a hidden
    // panel, and never reopen directly into a stale detail view.
    if (closingDetail) {
      endInspectionAndClearSelection();
  ...
  // Joint-class badges are intentionally neutral. Relationship colours are
  // reserved exclusively for Parent = blue and Child = red.
  ...
  function colorToRgba(hex, alpha) {
    ...
    if (!/^[0-9a-f]{6}$/i.test(value)) return `rgba(15, 23, 42, ${alpha})`;
  ...
  function capsule(text, color, options = {}) {
    ...
    badge.dataset.badgeKind = options.kind || 'generic';
    Object.assign(badge.style, {
      ...
      overflowWrap: 'anywhere', boxSizing: 'border-box', lineHeight: '1.25'
    });
    // Use explicit inline properties with !important so relation colors can never
    // be overwritten by joint-class styles, inherited rules or host-page CSS.
    badge.style.setProperty('border', `${strong ? '1.5px' : '1px'} solid ${color}`, 'important');
    badge.style.setProperty('background-color', colorToRgba(color, strong ? 0.13 : 0.08), 'important');
  ...
  function relationshipBadge(text, relationship) {
    ...
    const color = isChild ? RELATION_COLORS.child : RELATION_COLORS.parent;
    return capsule(text, color, { strong: true, kind: isChild ? 'child-relation' : 'parent-relation' });
  }
  function jointBadge(joint, text = null) {
    return capsule(text || jointType(joint), jointColor(joint), { strong: true, kind: 'joint-class' });
  }

  function relationshipChipsRow(label, values, relationship) {
    const row = document.createElement('div');
    const left = document.createElement('div');
    const right = document.createElement('div');
    Object.assign(row.style, {
    ...
    if (!list.length) {
      ...
      for (const value of list) right.appendChild(relationshipBadge(value, relationship));
    }
    row.append(left, right);
    return row;
  }
  ...
  function vecText(value) {
    ...
    return arr.slice(0, 3).map(raw => {
      ...
      const normalized = Math.abs(numeric) < 5e-10 ? 0 : numeric;
  ...
  function jointCard(joint) {
    ...
    Object.assign(card.style, {
      ...
      borderLeft: `6px solid ${RELATION_COLORS.parent}`, borderRadius: '13px', background: '#fff'
    });
  ...
  function childConnectionCard(connection) {
    ...
    Object.assign(card.style, {
      ...
      borderLeft: `6px solid ${RELATION_COLORS.child}`, borderRadius: '13px', background: '#fff'
    });
  ...
  function conciseGeometryRows(node = {}) {
    const { assets, totals } = technicalGeometryData(node);
    const rows = [];
    const visualObjects = Math.max(0, Number(node.directMeshCount ?? totals.instances) || 0);
    rows.push(['Visual objects', formatCount(visualObjects)]);
    rows.push(['Vertices', formatCount(totals.vertices)]);
    rows.push(['Triangles', formatCount(totals.triangles)]);

    const sizedAssets = assets.filter(asset => Array.isArray(asset?.stats?.boundsSize));
    if (sizedAssets.length === 1) {
      const dimensions = dimensionsText(sizedAssets[0].stats.boundsSize);
      if (dimensions !== '—') rows.push(['Dimensions', `${dimensions} m`]);
    }

    const assetFiles = assets.map(asset => `${asset.base || asset.assetKey || 'Visual'}${asset.ext ? '.' + asset.ext : ''}`).filter(Boolean);
    if (assetFiles.length) rows.push(['Asset', assetFiles.join(', ')]);

    const materials = [...new Set(assets.flatMap(asset => Array.isArray(asset?.stats?.materialNames) ? asset.stats.materialNames : []).filter(Boolean))];
    if (materials.length) rows.push(['Material', materials.join(', ')]);

    const textures = [...new Set(assets.flatMap(asset => Array.isArray(asset?.stats?.textureNames) ? asset.stats.textureNames : []).filter(Boolean))];
    rows.push(['Texture', textures.length ? 'Yes' : 'No']);
    return rows;
  }
  ...
  function componentSheetSections(record) {
    ...
    sections.push({ title: 'Overview', rows: [
      ['Parent', parent?.node?.name || parent?.linkName || 'Root'],
      ['Children', children.length ? children.map(c => c.name || c.linkName).join(', ') : 'None'],
      ['Objects in subtree', formatCount(node.objectCount ?? node.directMeshCount)],
      ['Direct joints', formatCount(directJoints.length)],
      ['Direct DOF', formatCount(node.directDOF)],
      ['Subtree joints', formatCount(node.subtreeJointCount)],
      ['Subtree DOF', formatCount(node.subtreeDOF)]
    ]});

    if (directJoints.length) {
      for (const joint of directJoints) {
        sections.push({
          title: `Joint to parent · ${joint.name || jointType(joint)}`,
          color: RELATION_COLORS.parent,
          rows: [
            ['Type', jointType(joint)],
            ['Axis', vecText(joint.axis)],
            ['Range', rangeText(joint)],
            ['Current', valueText(joint)]
          ]
        });
      }
    } else {
      sections.push({ title: 'Joints to parent', color: RELATION_COLORS.parent, rows: [['Connection', 'Fixed to parent']] });
    }

    if (childConnections.length) {
      sections.push({
        title: 'Child connections',
        color: RELATION_COLORS.child,
        rows: childConnections.map(connection => [
          connection.name || connection.linkName || 'Child',
          `${connection.joints?.length ? connection.joints.map(jointType).join(' + ') : 'Fixed'} · ${connection.dof || 0} DOF`
        ])
      });
    }

    const physicsRows = [];
    if (physics.explicitInertial && Number.isFinite(Number(physics.mass))) {
      physicsRows.push(['Mass', `${Number(physics.mass).toFixed(6).replace(/\.?0+$/, '')} kg`]);
    }
    if (physics.explicitInertial && physics.centerOfMass) {
      const center = vecText(physics.centerOfMass);
      if (center !== '—') physicsRows.push(['Center of mass', `${center} m`]);
    }
    if (physicsRows.length) sections.push({ title: 'Physics', rows: physicsRows });
  ...
  function safeImageSource(image, fallback = '') {
    const raw = String(image?.getAttribute?.('src') || '').trim();
    if (/^(?:data:image\/|blob:|https?:\/\/)/i.test(raw)) return raw;
    const cached = String(fallback || '').trim();
    if (/^(?:data:image\/|blob:|https?:\/\/)/i.test(cached)) return cached;
    return '';
  }
  ...
  function cloneForSnapshot(node) {
    ...
    do {
      ...
      if (el.tagName === 'IMG') {
        ...
        if (!/^(data:image\/|blob:|https?:\/\/)/i.test(src)) el.removeAttribute('src');
      }
    } while (walker.nextNode());
    return clone;
  }
  function serializeNodeAsSvgDataUrl(node, width, height) {
    const serializer = new XMLSerializer();
    const xhtml = serializer.serializeToString(node);
    const svg = `<?xml version="1.0" encoding="UTF-8"?>`
      + `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">`
      + `<foreignObject x="0" y="0" width="100%" height="100%">`
      + `<div xmlns="http://www.w3.org/1999/xhtml" style="width:${width}px;height:${height}px;background:#f8fbfb;padding:0;margin:0;">${xhtml}</div>`
      + `</foreignObject></svg>`;
    return 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg);
  }
  async function renderNodeToPngBlob(node, scale = 2) {
    const width = Math.max(640, Math.ceil(node.scrollWidth || node.getBoundingClientRect().width || 640));
    const height = Math.max(400, Math.ceil(node.scrollHeight || node.getBoundingClientRect().height || 400));
    const clone = cloneForSnapshot(node);
    Object.assign(clone.style, {
      width: `${width}px`, minWidth: `${width}px`, maxWidth: `${width}px`,
      height: 'auto', overflow: 'visible', background: '#f8fbfb',
      padding: '0', margin: '0'
    });
    const svgUrl = serializeNodeAsSvgDataUrl(clone, width, height);
    const image = await imageFromSource(svgUrl);
  ...
  async function downloadTechnicalSheet(record) {
    ...
    if (detailRecord !== record || viewMode !== 'detail') buildDetail(record);
    const target = ui.detailView;
    const blob = await renderNodeToPngBlob(target, 2);
    if (!blob) return false;
    const componentName = record.node?.name || record.linkName || 'component';
  ...
  function buildDetail(record) {
    ...
    img.src = safeImageSource(record.thumb, thumbnailCache.get(record.linkName) || '') || placeholderDataUrl(node.name || record.linkName);
    ...
    ui.detailView.appendChild(relationshipChipsRow('Parent', [parent?.node?.name || parent?.linkName || 'Root'], 'parent'));
    ui.detailView.appendChild(relationshipChipsRow('Children', children.map(c => c.name || c.linkName).filter(Boolean), 'child'));
    ui.detailView.appendChild(infoRow('Objects in subtree', String(Math.max(0, Number(node.objectCount ?? node.directMeshCount) || 0))));
    ui.detailView.appendChild(infoRow('Direct joints', String(directJoints.length)));
    ui.detailView.appendChild(infoRow('Direct DOF', String(Math.max(0, Number(node.directDOF) || 0))));
    ui.detailView.appendChild(infoRow('Subtree joints', String(Math.max(0, Number(node.subtreeJointCount) || 0))));
    ui.detailView.appendChild(infoRow('Subtree DOF', String(Math.max(0, Number(node.subtreeDOF) || 0))));
    ...
    const hasMass = physics?.explicitInertial && Number.isFinite(Number(physics.mass));
    const centerOfMass = physics?.explicitInertial ? vecText(physics.centerOfMass) : '—';
    if (hasMass || centerOfMass !== '—') {
      ui.detailView.appendChild(sectionTitle('Physics'));
      if (hasMass) ui.detailView.appendChild(infoRow('Mass', `${Number(physics.mass).toFixed(6).replace(/\.?0+$/, '')} kg`));
      if (centerOfMass !== '—') ui.detailView.appendChild(infoRow('Center of mass', `${centerOfMass} m`));
    }
  ...
  function endInspectionAndClearSelection() {
    // startComponentInspection already normalizes the complete model instantly.
    // Back therefore only ends the inspection transaction. It must not call
    // Show All, change the camera, start a visibility tween or touch Tools state.
    try { app.endComponentInspection?.(); } catch (_) {}
    try { app.clearSelection?.(); app.interaction?.clearHover?.(); } catch (_) {}
    clearSelectionBubble();
  }
  function returnToTreeFromDetail() {
    endInspectionAndClearSelection();
    showTree({ revealSelection: false });
  }

  function ensureTreeState({ revealSelection = true } = {}) {
    if (viewMode === 'detail' || app.getComponentInspectionState?.()) {
      endInspectionAndClearSelection();
    }
    showTree({ revealSelection });
  }
  ...
  ui.btn.addEventListener('click', ev => {
    ...
    if (open) setPanelOpen(false);
      ...
      ensureTreeState({ revealSelection: true });
      setPanelOpen(true);
    }
  });
  ...
  ui.downloadSheetBtn.addEventListener('click', async ev => {
    ev.preventDefault();
    ev.stopPropagation();
    if (!detailRecord || technicalReportBusy) return;
    technicalReportBusy = true;
    try { await downloadTechnicalSheet(detailRecord); }
    catch (error) { console.error('[Component technical report] failed:', error); alert('The technical report could not be downloaded.'); }
    finally { technicalReportBusy = false; }
  });
  ...
  return {
    open: () => { ensureTreeState({ revealSelection: true }); setPanelOpen(true); },
    close: () => setPanelOpen(false),
    set: isOpen => { if (isOpen) { ensureTreeState({ revealSelection: true }); setPanelOpen(true); } else setPanelOpen(false); },
