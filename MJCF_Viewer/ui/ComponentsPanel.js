// AutoMind MJCF body-tree Components panel — BUILD248.
// BUILD248:
...
export function createComponentsPanel(app, theme) {
  ...
  function setPanelOpen(isOpen) {
    ...
    if (!open && wasDetail) {
      // Closing the technical sheet is the explicit lifetime boundary for the
      // persistent red child-body highlighting.
      try { app.clearChildHighlights?.(); } catch (_) {}
    }
  ...
  function highlightRecordChildren(record) {
    const childNames = (Array.isArray(record?.node?.children) ? record.node.children : [])
      .map(child => String(child?.linkName || child?.name || ''))
      .filter(Boolean);
    try { app.highlightChildBodies?.(childNames); } catch (_) {}
  }
  ...
  function selectBodyRecord(record, { openDetails = true } = {}) {
    ...
    highlightRecordChildren(record);
  ...
  function jointBadge(joint, text = null) {
    ...
    badge.textContent = text || jointType(joint);
    Object.assign(badge.style, {
      display: 'inline-flex', alignItems: 'center', width: 'fit-content', padding: '3px 8px', borderRadius: '999px',
      border: `1px solid ${color}`, background: `${color}18`, color, fontSize: '9px', fontWeight: '900', letterSpacing: '.02em'
    });
    return badge;
  }
  ...
  function jointCard(joint) {
    ...
    Object.assign(header.style, { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', marginBottom: '8px' });
    title.textContent = joint?.name || jointType(joint);
    Object.assign(title.style, { fontWeight: '900', color: theme.text, fontSize: '12px', overflowWrap: 'anywhere' });
    header.append(title, jointBadge(joint));
  ...
  function childConnectionCard(connection) {
    ...
    heading.textContent = connection?.name || connection?.linkName || 'Child';
    Object.assign(heading.style, { color: theme.text, fontWeight: '850', fontSize: '11px', overflowWrap: 'anywhere' });
  ...
  function componentSheetSections(record) {
    ...
    if (directJoints.length) {
      for (const joint of directJoints) sections.push({ title: `Joint · ${joint.name || jointType(joint)}`, color: jointColor(joint), rows: [
    ...
    } else sections.push({ title: 'Joints to parent', color: JOINT_CLASS_COLORS.Fixed, rows: [['Connection', 'Fixed to parent'], ['Class color', JOINT_CLASS_COLORS.Fixed.toUpperCase()], ['Degrees of freedom', '0']] });
    sections.push({ title: 'Child connections', rows: childConnections.length
