// AutoMind MJCF body-tree Components panel — BUILD242.
// BUILD242:
...
export function createComponentsPanel(app, theme) {
  ...
  [ui.btn, ui.backBtn, ui.showAllBtn].forEach(button => { button.type = 'button'; });
  ...
  function revealRecord(record, scroll = false) {
    ...
    if (scroll && open && viewMode === 'tree') {
      requestAnimationFrame(() => {
        ...
        const desiredTop = container.scrollTop + (rowRect.top - containerRect.top) - (container.clientHeight - rowRect.height) * 0.5;
  ...
  function jointCard(joint) {
    ...
    Object.assign(card.style, { padding: '11px 12px', margin: '7px 0', border: `1px solid ${theme.stroke}`, borderRadius: '13px', background: '#fff' });
    title.textContent = joint?.name || jointType(joint);
    Object.assign(title.style, { fontWeight: '900', color: theme.text, fontSize: '12px', marginBottom: '7px', overflowWrap: 'anywhere' });
  ...
  function buildDetail(record) {
    ...
    else ui.detailView.appendChild(infoRow('Connection', 'Fixed to parent'));
    ...
    if (childConnections.length) {
      for (const connection of childConnections) {
        const labels = connection.joints?.length ? connection.joints.map(jointType).join(' + ') : 'Fixed';
        ui.detailView.appendChild(infoRow(connection.name || connection.linkName || 'Child', `${labels} · ${connection.dof || 0} DOF`));
      }
    ...
    if (geometry) ui.detailView.appendChild(infoRow('Collision-only geoms', String(Math.max(0, Number(geometry.collisionCount) || 0))));
    const visualFiles = assets.map(a => `${a.base || a.assetKey || 'Visual'}${a.ext ? '.' + a.ext : ''}`);
    const details = document.createElement('details');
    const summary = document.createElement('summary');
    summary.textContent = `Technical geometry (${visualFiles.length})`;
    Object.assign(details.style, { marginTop: '8px', padding: '10px 12px', border: `1px solid ${theme.stroke}`, borderRadius: '12px', background: '#fff' });
    Object.assign(summary.style, { cursor: 'pointer', fontWeight: '850', color: '#0f766e', fontSize: '11px' });
    details.appendChild(summary);
    const list = document.createElement('div');
    Object.assign(list.style, { paddingTop: '9px', color: theme.text, fontWeight: '700', fontSize: '10px', lineHeight: '1.6', overflowWrap: 'anywhere' });
    list.textContent = visualFiles.length ? visualFiles.join('\n') : 'Procedural or inherited geometry';
