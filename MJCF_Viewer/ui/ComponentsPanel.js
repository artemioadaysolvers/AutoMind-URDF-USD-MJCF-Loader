...
// - Tree rows contain only chevron, thumbnail and body name.
// - No Geometry folders, mesh counts, child counts or joint badges in the tree.
...
export function createComponentsPanel(app, theme) {
  ...
  function makeBodyRow(node, depth, ancestors, thumbnailJobs, expandByDefault = false) {
    ...
    Object.assign(row.style, {
      display: 'grid', gridTemplateColumns: '24px 72px minmax(0,1fr)', alignItems: 'center', gap: '10px',
      minHeight: '66px', padding: `7px 10px 7px ${8 + depth * 18}px`, margin: '4px 0',
      border: '1px solid transparent', borderRadius: '14px', cursor: 'pointer', color: theme.text,
    ...
    Object.assign(chevronButton.style, {
      width: '22px', height: '34px', border: 'none', background: 'transparent', color: '#0f766e',
      fontSize: '17px', fontWeight: '900', cursor: 'pointer', padding: '0'
    });
    Object.assign(thumb.style, {
      width: '72px', height: '54px', objectFit: 'contain', borderRadius: '10px', border: `1px solid ${theme.stroke}`,
    ...
    Object.assign(title.style, {
      fontWeight: '800', fontSize: '13px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'
    });

    row.append(chevronButton, thumb, title);
    wrap.append(row, childrenEl);

    const childCount = Array.isArray(node.children) ? node.children.length : 0;
    const record = {
      kind: 'body', row, wrap, childrenEl, chevron: chevronButton, thumb,
  ...
  function buildDetail(record) {
    ...
    focusBtn.textContent = 'Focus component';
    showAllBtn.textContent = 'Show complete model';
    Object.assign(actions.style, { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '18px' });
    for (const btn of [focusBtn, showAllBtn]) {
      Object.assign(btn.style, {
        padding: '10px 12px', borderRadius: '12px', border: `1px solid ${theme.stroke}`,
        background: '#fff', color: theme.text, fontWeight: '800', cursor: 'pointer'
      });
      btn.addEventListener('mouseenter', () => { btn.style.background = theme.tealFaint || '#f0fdfa'; btn.style.borderColor = '#0ea5a6'; });
      btn.addEventListener('mouseleave', () => { btn.style.background = '#fff'; btn.style.borderColor = theme.stroke; });
    }
    ...
    actions.append(focusBtn, showAllBtn);
    ui.detailView.appendChild(actions);
  }
