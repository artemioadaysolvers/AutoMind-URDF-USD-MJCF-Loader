// AutoMind MJCF body-tree Components panel — BUILD246.
// BUILD246:
...
export function createComponentsPanel(app, theme) {
  ...
  function componentSheetSections(record) {
    ...
    if (directJoints.length) {
      for (const joint of directJoints) sections.push({ title: `Joint · ${joint.name || jointType(joint)}`, rows: [
    ...
    } else sections.push({ title: 'Joints to parent', rows: [['Connection', 'Fixed to parent']] });
  ...
  async function downloadTechnicalSheet(record) {
    ...
    for (const section of sections) {
      ctx.fillStyle = '#0f766e'; ctx.font = '900 30px Arial';
      ctx.fillText(String(section.title || '').toUpperCase(), margin, y);
      y += 28;
