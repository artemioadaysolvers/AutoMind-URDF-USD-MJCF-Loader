...
export function createComponentsPanel(app, theme) {
  ...
  function componentSheetSections(record) {
    ...
    sections.push({ title: 'Geometry', rows: conciseGeometryRows(node) });
    return sections;
  }
  ...
  async function downloadTechnicalSheet(record) {
    ...
    const image = await imageFromSource(safeImageSource(record.thumb, thumbnailCache.get(record.linkName) || ''));
    if (image) {
  ...
  function buildDetail(record) {
    ...
    ui.detailView.appendChild(sectionTitle('Geometry'));
    for (const [label, value] of conciseGeometryRows(node)) ui.detailView.appendChild(infoRow(label, value));
  }
