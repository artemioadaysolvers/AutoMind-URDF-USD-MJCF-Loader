...
export function createComponentsPanel(app, theme) {
  ...
  // BUILD238: the panel is usable immediately. Thumbnail generation is an
  // independent background activity and must never lock this control.
  ...
  async function loadThumbnailJob(job, generation) {
    ...
    try {
      // A malformed asset or stalled offscreen render must not stop the entire
      // warm-up queue. The underlying thumbnailer also has its own timeout.
      const url = await promiseWithTimeout(
        Promise.resolve(app.assets.thumbnail?.(job.assetKey)),
        4500,
        ''
      );
      if (disposed || generation !== thumbnailGeneration || !job.img.isConnected) return false;
  ...
  function warmThumbnails(thumbnailJobs) {
    ...
    thumbnailWarmupPromise = (async () => {
      ...
      for (let i = 0; i < jobs.length; i++) {
        ...
        if (disposed || generation !== thumbnailGeneration) return false;
        // Progress belongs inside the panel. Never replace or disable the main
        // Components button while images are being prepared.
      ...
      if (!disposed && generation === thumbnailGeneration) {
        ...
        setTimeout(() => { if (!disposed && generation === thumbnailGeneration) ui.status.style.display = 'none'; }, 900);
      }
      return false;
    });
  ...
  async function preload() {
    ...
    preloadPromise = (async () => {
      ...
      // Resolve readiness as soon as the hierarchy exists. Thumbnails continue
      // independently, so users may open, expand and select components at once.
      if (jobs.length) {
        ui.status.textContent = `Loading thumbnails… 0/${jobs.length}`;
        warmThumbnails(jobs);
      } else {
        ui.status.textContent = 'Components ready';
...
function promiseWithTimeout(promise, timeoutMs, fallbackValue) {
  return new Promise(resolve => {
    ...
    const timer = setTimeout(() => finish(fallbackValue), Math.max(250, Number(timeoutMs) || 4500));
