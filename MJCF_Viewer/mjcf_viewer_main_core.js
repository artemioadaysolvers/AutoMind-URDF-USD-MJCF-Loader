...
// BUILD249: high-resolution component sheets, complete technical geometry, persistent thumbnails in exported HTML and Show All on Back.
...
export function render(opts = {}) {
  ...
  debugLog('render init', {
    mode: 'MJCF BUILD253_LOCAL_OBJ_SAFE_SHOW_ALL',
    selectMode,
    IA_Widgets,
  ...
  function cloneInspectionMaterial(material, role = 'background') {
    // Inspection colours are applied directly to the real component meshes.
    // Parent and child bodies are solid colours, not translucent duplicate-
    // geometry overlays. The remaining mechanism stays neutral and X-Ray.
    const isParent = role === 'parent';
    const isChild = role === 'child';
    const clone = new THREE.MeshBasicMaterial({
      color: isParent ? 0x0066ff : (isChild ? 0xff102f : 0x9aa3ad),
      transparent: !(isParent || isChild),
      opacity: (isParent || isChild) ? 1.0 : 0.22,
      depthWrite: !!(isParent || isChild),
      depthTest: true,
  ...
  function startComponentInspection(parentLinkName = '', childLinkNames = []) {
    ...
    // Render mode applies X-Ray globally; restore the selected parent/children as
    // fully opaque colours afterwards.
    for (const rec of meshes) {
      if (rec.role !== 'parent' && rec.role !== 'child') continue;
  ...
  if (typeof window !== 'undefined') {
    ...
    window.MJCFViewer.__build = 'BUILD249_COMPONENT_INSPECTION_XRAY';
