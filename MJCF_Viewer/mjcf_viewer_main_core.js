...
// BUILD240: clean MJCF body hierarchy, component detail view, HTML download, original hover, remote audio and on-demand rendering.
...
function buildComponentTree(robot, assetToMeshes) {
  ...
  for (const joint of jointCandidates) {
    ...
    const old = jointByChild.get(child);
    const technical = String(joint?.jointType || joint?.type || '').includes('component');
    if (!old || (!technical && String(old?.jointType || old?.type || '').includes('component'))) {
  ...
  function annotateObjectCounts(node, seen = new Set()) {
    ...
    seen.add(node);
    let total = Math.max(0, Number(node.directMeshCount) || 0);
    for (const child of node.children || []) total += annotateObjectCounts(child, seen);
    node.objectCount = total;
    return total;
  }
...
export function render(opts = {}) {
  ...
  debugLog('render init', {
    mode: 'MJCF BUILD241_COMPONENT_TREE_DOWNLOAD_FIX',
    selectMode,
    IA_Widgets,
  ...
  const app = {
    ...
    assets: {
      ...
      bodyThumbnail: (linkName) => thumbs?.thumbnail(`__body__:${String(linkName || '')}`) || '',
    },
    isolate: {
  ...
  app.ready = (async () => {
    try {
      ...
      debugLog('MJCF loaded BUILD241_COMPONENT_TREE_DOWNLOAD_FIX', {
        robot: robot.name,
  ...
  if (typeof window !== 'undefined') {
    ...
    window.MJCFViewer.__build = 'BUILD240_COMPONENT_DETAILS_HTML_DOWNLOAD';
