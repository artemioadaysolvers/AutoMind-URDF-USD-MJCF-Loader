...
export function render(opts = {}) {
  ...
  debugLog('render init', {
    mode: 'MJCF BUILD249_COMPONENT_INSPECTION_XRAY',
    selectMode,
    IA_Widgets,
  ...
  function cloneInspectionMaterial(material) {
    ...
    if (!clone || clone === material) return clone;
    clone.userData = { ...(clone.userData || {}), __automindInspectionMaterial: true };
    if (clone.color?.setHex) clone.color.setHex(0x9aa3ad);
    if (clone.emissive?.setHex) clone.emissive.setHex(0x000000);
    for (const slot of ['map','alphaMap','aoMap','bumpMap','normalMap','emissiveMap','roughnessMap','metalnessMap','specularMap','envMap','lightMap','displacementMap']) {
    ...
    if ('side' in clone && THREE.FrontSide !== undefined) clone.side = THREE.FrontSide;
