...
// BUILD264: high-resolution component sheets, complete technical geometry, persistent thumbnails in exported HTML and Show All on Back.
...
export function render(opts = {}) {
  ...
  debugLog('render init', {
    mode: 'MJCF BUILD264_SECTION_STATE_BLACK_TEXTURE_FIX',
    selectMode,
    IA_Widgets,
  ...
  function endComponentInspection() {
    ...
    if (state?.meshes) {
      for (const rec of state.meshes) {
        ...
        const active = Array.isArray(rec.mesh.material) ? rec.mesh.material : [rec.mesh.material];
        for (const mat of active) {
          if (mat?.userData?.__automindInspectionMaterial) { try { mat.dispose?.(); } catch (_) {} }
        }
        rec.mesh.material = rec.material;
      }
    }
    try { tools?.setRenderMode?.(state?.previousMode || 'Solid'); } catch (_) {}
    // If Section was enabled while the detail/inspection materials were active,
    // its cached materials belong to the temporary inspection view. Rebuild the
    // clipping materials from the restored CAD materials without changing the
    // Enable Section checkbox or plane settings.
  ...
  function startComponentInspection(parentLinkName = '', childLinkNames = []) {
    ...
    showAllWithoutCamera(core);
    const previousMode = tools?.getRenderMode?.() || core.__currentRenderMode || 'Solid';
    ...
    componentInspectionState = { previousMode, meshes, parentLinkName, childLinkNames: childNames };
    try { tools?.setRenderMode?.('X-Ray'); } catch (_) {}
    // Keep every inspection material translucent. Parent and child components
    // are distinguished only by blue/red colour, not by becoming solid.
  ...
  const app = {
    ...
    getComponentInspectionState: () => componentInspectionState ? {
      ...
      previousMode: componentInspectionState.previousMode
    } : null,
    selectBody(linkName = '') {
  ...
  if (typeof window !== 'undefined') {
    ...
    window.MJCFViewer.__build = 'BUILD264_COMPONENT_INSPECTION_XRAY';
