...
export function render(opts = {}) {
  ...
  function cloneInspectionMaterial(material) {
    // Use a fresh, texture-free material instead of cloning arbitrary CAD
    // materials. Cloned physical materials can retain optional texture slots with
    // incomplete matrix state; Three r132 then throws in refreshMaterialUniforms
    // when Show all renders them. A clean MeshBasicMaterial is deterministic.
  ...
  function startComponentInspection(parentLinkName = '', childLinkNames = []) {
    ...
    robot.traverse?.(mesh => {
      ...
      const replacements = originals.map(cloneInspectionMaterial);
      meshes.push({ mesh, material: original });
      mesh.material = Array.isArray(original) ? replacements : replacements[0];
    });
    componentInspectionState = { previousMode, meshes, parentLinkName, childLinkNames: childNamesForInspection(childLinkNames) };
    try { tools?.setRenderMode?.('X-Ray'); } catch (_) {}
    try { inter?.highlightInspection?.(parentLinkName, componentInspectionState.childLinkNames); } catch (_) {}
