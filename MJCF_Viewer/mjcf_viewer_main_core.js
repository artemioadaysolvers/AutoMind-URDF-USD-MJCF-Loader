...
// BUILD236: upright MJCF orientation, hierarchical preloaded components, continuous camera tweens, pooled hover and on-demand rendering.
...
function buildComponentTree(robot, assetToMeshes) {
  ...
  Object.keys(robot.links || {}).forEach((name, index) => linkOrder.set(name, index));
  ...
  assetToMeshes.forEach((meshes, assetKey) => {
    ...
    for (const [linkName, groupMeshes] of grouped) {
      ...
      list.push({ assetKey, base, ext: ext || 'visual', count: groupMeshes.length, linkName });
  ...
  for (const linkName of assetsByLink.keys()) {
    ...
    let cursor = linkName;
    const seen = new Set();
    while (cursor && cursor !== '__world__' && !seen.has(cursor)) {
      seen.add(cursor);
      needed.add(cursor);
      cursor = robot.parentLinkByLink?.get?.(cursor) || '__world__';
    }
  }
  ...
  for (const linkName of needed) {
    nodes.set(linkName, {
      type: 'link', name: linkName, linkName,
      assets: (assetsByLink.get(linkName) || []).sort((a, b) => a.base.localeCompare(b.base, undefined, { numeric: true, sensitivity: 'base' })),
      children: []
    });
  }
  ...
  for (const [linkName, node] of nodes) {
    let parentName = robot.parentLinkByLink?.get?.(linkName) || '__world__';
    while (parentName && parentName !== '__world__' && !nodes.has(parentName)) {
      parentName = robot.parentLinkByLink?.get?.(parentName) || '__world__';
    }
    if (parentName && parentName !== '__world__' && nodes.has(parentName)) nodes.get(parentName).children.push(node);
    else roots.push(node);
  }

  const worldAssets = assetsByLink.get('__world__') || [];
  if (worldAssets.length) roots.unshift({ type: 'link', name: 'World', linkName: '__world__', assets: worldAssets, children: [] });
  ...
  const sortNodes = list => {
    list.sort((a, b) => {
      ...
      return ia - ib || a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' });
    });
...
export function render(opts = {}) {
  ...
  debugLog('render init', {
    mode: 'MJCF BUILD236_UPRIGHT_HIERARCHY_PRELOAD_CAMERA_TWEEN',
    selectMode,
    IA_Widgets,
    ...
    selectAsset(assetKey, preferredLinkName = '') {
      ...
      for (const mesh of ordered) {
        ...
        if (linkName && inter?.selectLink?.(linkName)) {
          try { comps?.selectComponent?.(assetKey, linkName, { reveal: true, openPanel: false }); } catch (_) {}
          return true;
        }
      }
      return false;
  ...
  app.ready = (async () => {
    try {
      ...
      // BUILD236: start the component hierarchy and thumbnail warm-up as soon
      // as the visible model has completed its first stable frames. The panel
      // button remains disabled until every thumbnail is cached, so opening the
      // panel never triggers a large burst of rendering work.
      ...
      debugLog('MJCF loaded BUILD236_UPRIGHT_HIERARCHY_PRELOAD_CAMERA_TWEEN', {
        robot: robot.name,
  ...
  if (typeof window !== 'undefined') {
    ...
    window.MJCFViewer.__build = 'BUILD236_UPRIGHT_HIERARCHY_PRELOAD_CAMERA_TWEEN';
