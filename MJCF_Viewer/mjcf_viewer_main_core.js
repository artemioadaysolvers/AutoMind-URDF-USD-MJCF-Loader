...
// BUILD266: high-resolution component sheets, complete technical geometry, persistent thumbnails in exported HTML and Show All on Back.
...
function repairTexturedMaterial(mat) {
  ...
  // Legacy BUILDs stored a Texture in userData. Material.clone() converted it to
  // JSON and later restored that plain object into map. Accept only real textures.
  const legacy = ud.__automindTextureMap;
  const canonical = mat.__automindTextureMapRef;
  if (canonical?.isTexture) {
    mat.map = canonical;
  } else if (mat.map?.isTexture) {
    try {
      Object.defineProperty(mat, '__automindTextureMapRef', {
        value: mat.map, writable: true, configurable: true, enumerable: false
      });
    } catch (_) { mat.__automindTextureMapRef = mat.map; }
  } else if (legacy?.isTexture) {
    mat.map = legacy;
    try {
      Object.defineProperty(mat, '__automindTextureMapRef', {
        value: legacy, writable: true, configurable: true, enumerable: false
      });
    } catch (_) { mat.__automindTextureMapRef = legacy; }
  } else if (mat.map) {
    // A non-Texture map can never be uploaded by Three.js safely.
    mat.map = null;
  }
  try { delete ud.__automindTextureMap; } catch (_) {}
...
function materialFallbackColor(material) {
  const mapData = material?.map?.userData || {};
  const flat = mapData.__automindFlatTextureColor || material?.userData?.__automindFlatTextureColor || null;
  const mapStats = mapData.__automindTextureStats || material?.userData?.__automindTextureStats || null;
  const mean = Array.isArray(mapStats?.mean) ? mapStats.mean : null;
  const hasAuthoredFlat = Array.isArray(flat) && flat.length >= 3 && Number(flat[3] ?? 1) > 0.05;
  let rgb = null;
  if (hasAuthoredFlat) {
    // Preserve exact CAD colour swatches, including intentional pure black.
    rgb = flat.slice(0, 3);
  } else if (mean && mean.length >= 3 && mean.slice(0, 3).some(v => Number(v) > 0.035)) {
    rgb = mean.slice(0, 3);
  ...
  if (!hasAuthoredFlat && color.r < 0.025 && color.g < 0.025 && color.b < 0.025) color.setHex(0x9fb6bf);
  return color;
}
...
function createViewportTextureProxy(core, robot, enabled = true, bakeVertexColors = true) {
  ...
  robot.traverse?.((source) => {
    ...
    const proxyMats = sourceMats.map((src, materialIndex) => {
      ...
      const map = src?.map?.isTexture ? src.map : (src?.__automindTextureMapRef?.isTexture ? src.__automindTextureMapRef : null);
...
function showAllWithoutCamera(core) {
  ...
  const meshes = collectRobotMeshes(core.robot);
  animateMeshVisibility(core, meshes, true, 0, () => restoreAllMeshesToCurrentRenderMode(core));
...
export function render(opts = {}) {
  ...
  debugLog('render init', {
    mode: 'MJCF BUILD266_TOOLS_COMPONENTS_STATE_SAFE',
    selectMode,
    IA_Widgets,
  ...
  let tools = null;
  let comps = null;
  let componentInspectionState = null;
  ...
  function childNamesForInspection(values = []) {
    return (Array.isArray(values) ? values : [values]).map(value => String(value || '')).filter(Boolean);
  }
  function cloneInspectionMaterial(material, role = 'background') {
    // The complete mechanism remains X-Ray during component inspection.
    // Parent and child bodies keep their blue/red identity, but are translucent
    // like the neutral gray background rather than becoming solid surfaces.
    const isParent = role === 'parent';
    const isChild = role === 'child';
    const clone = new THREE.MeshBasicMaterial({
      color: isParent ? 0x0066ff : (isChild ? 0xff102f : 0x9aa3ad),
      transparent: true,
      opacity: (isParent || isChild) ? 0.48 : 0.22,
      depthWrite: false,
      depthTest: true,
    ...
    clone.name = '__automind_component_inspection__';
    clone.userData = {
      __automindInspectionMaterial: true,
      __automindInspectionRole: role,
      __automindBaseOpacity: 1,
      __automindBaseTransparent: false,
      __automindBaseDepthWrite: true,
      __automindBaseDepthTest: true
    };
    if (Array.isArray(material?.clippingPlanes) && material.clippingPlanes.length) {
      clone.clippingPlanes = material.clippingPlanes.slice();
      clone.clipIntersection = !!material.clipIntersection;
      clone.clipShadows = material.clipShadows !== false;
    }
    clone.needsUpdate = true;
    return clone;
  }
  function endComponentInspection() {
    const state = componentInspectionState;
    componentInspectionState = null;

    // Restore every authored material first, while no disposal can occur. This is
    // one atomic synchronous transaction, so the renderer never sees a disposed
    // material still attached to a mesh.
    const temporaryMaterials = new Set();
    if (state?.meshes) {
      for (const rec of state.meshes) {
        if (!rec?.mesh) continue;
        for (const mat of (Array.isArray(rec.mesh.material) ? rec.mesh.material : [rec.mesh.material])) {
          if (mat?.userData?.__automindInspectionMaterial) temporaryMaterials.add(mat);
        }
    ...
    // Preserve the CURRENT Tools state. Components no longer changes the global
    // render-mode selector, so a mode selected before or during inspection remains.
    try { tools?.refreshRenderMode?.(); } catch (_) {}
    try { tools?.refreshSection?.(); } catch (_) {}

    for (const mat of temporaryMaterials) { try { mat.dispose?.(); } catch (_) {} }
    try { core.syncKinematicPresentation?.({ force: true }); } catch (_) {}
    try { core.invalidate?.(); } catch (_) {}
  }

  function showAllDuringComponentInspection() {
    // Inspection already keeps the complete mechanism visible. Do not run the
    // visibility/material tween when Show all is pressed: it is unnecessary and
    // used to touch temporary inspection materials while Three was refreshing
    // uniforms. Only normalize visibility and animate the camera to Iso.
    if (!robot) return false;
    core.__componentViewFocusMeshes = null;
    core.__componentViewFocusObject = null;
    try { core.setMechanismSuppressed?.(false, 0); } catch (_) {}
    robot.traverse?.(mesh => {
      if (!mesh?.isMesh || !mesh.geometry || mesh.userData?.__isHoverOverlay) return;
      mesh.visible = true;
      mesh.userData.__automindVisibilityTarget = true;
    });
    try { inter?.clearHover?.(); inter?.refreshSelectionMarker?.(); } catch (_) {}
    viewPreset(core, 'iso', robot, 780);
    try { core.syncKinematicPresentation?.({ force: true }); } catch (_) {}
    try { core.invalidate?.(); } catch (_) {}
    return true;
  }

  function startComponentInspection(parentLinkName = '', childLinkNames = []) {
    endComponentInspection();
    if (!robot || !parentLinkName) return false;

    // Cancel any isolate/show-all fade before recording materials. A pending RAF
    // must never keep mutating materials that Components has temporarily replaced.
    restoreAllMeshesToCurrentRenderMode(core);

    const entryMode = tools?.getRenderMode?.() || core.__currentRenderMode || 'Solid';
    const childNames = childNamesForInspection(childLinkNames);
    const childSet = new Set(childNames);
    const meshes = [];
    robot.traverse?.(mesh => {
      if (!mesh?.isMesh || !mesh.geometry || mesh.userData?.__isHoverOverlay) return;
      const linkName = linkNameFromObject(mesh);
      const role = linkName === parentLinkName ? 'parent' : (childSet.has(linkName) ? 'child' : 'background');
      const original = mesh.material;
      const originals = Array.isArray(original) ? original : [original];
      const replacements = originals.map(material => cloneInspectionMaterial(material, role));
      meshes.push({ mesh, material: original, role });
      mesh.material = Array.isArray(original) ? replacements : replacements[0];
    });
    componentInspectionState = { entryMode, meshes, parentLinkName, childLinkNames: childNames };

    // Inspection materials already encode neutral/blue/red translucent roles.
    // Do not change the global Tools render mode behind the user's back.
    try { inter?.setHoverExcludedLinks?.([parentLinkName, ...childNames]); } catch (_) {}
  ...
  const app = {
    ...
    startComponentInspection,
    endComponentInspection,
    getComponentInspectionState: () => componentInspectionState ? {
      parentLinkName: componentInspectionState.parentLinkName,
      childLinkNames: componentInspectionState.childLinkNames.slice(),
      entryMode: componentInspectionState.entryMode,
      currentMode: tools?.getRenderMode?.() || core.__currentRenderMode || 'Solid'
    } : null,
    selectBody(linkName = '') {
    ...
    isolate: {
      ...
      clear: () => componentInspectionState ? showAllDuringComponentInspection() : showAll(core),
    },
    showAll: () => componentInspectionState ? showAllDuringComponentInspection() : showAll(core),
  ...
  app.ready = (async () => {
    try {
      ...
      debugLog('MJCF loaded BUILD248_COMPONENT_TECHNICAL_SHEETS', {
        robot: robot.name,
  ...
  if (typeof window !== 'undefined') {
    ...
    window.MJCFViewer.__build = 'BUILD266_TOOLS_COMPONENTS_STATE_SAFE';
  ...
  return Object.assign(app, {
    resize: core.resize,
    destroy() {
      try { endComponentInspection(); } catch (_) {}
