...
// MJCF BUILD227 interaction: bounded drag commits, capture-loss finalization and synchronized closed chains.
/* global THREE */
...
export function attachInteraction({ scene, camera, renderer, controls, robot, selectMode = 'link', getSectionPlane = null, onSelectLink = null, onKinematicCommit = null }) {
  ...
  function endJointDrag(ev) {
    ...
    try {
      ...
      // Drain the last coalesced pointer target through the same bounded steps used
      // during dragging.  This prevents a large release snap while still ending on
      // the most recent pointer position.
      let guard = 0;
      while (activeDrag?.pendingTarget != null && guard++ < 24) {
        const before = getJointValue(activeDrag.joint);
        const accepted = flushPendingJointCommit({ force: true });
        const after = getJointValue(activeDrag.joint);
        if (!accepted || Math.abs(after - before) < 1e-10) break;
      }
