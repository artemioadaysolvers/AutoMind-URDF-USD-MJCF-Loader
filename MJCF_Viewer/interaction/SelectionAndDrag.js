...
// MJCF BUILD249 interaction: original translucent duplicate-geometry hover, indexed picking and nonblocking drag commits.
/* global THREE */
...
const INSPECTION_PARENT_COLOR = 0x0066ff;
const INSPECTION_PARENT_OPACITY = 0.72;
const CHILD_HIGHLIGHT_COLOR = 0xff102f;
const CHILD_HIGHLIGHT_OPACITY = 0.68;
...
export function attachInteraction({ scene, camera, renderer, controls, robot, selectMode = 'link', getSectionPlane = null, onSelectLink = null, onKinematicCommit = null, invalidate = null }) {
  ...
  const parentHighlight = buildHoverOverlay({
    color: INSPECTION_PARENT_COLOR,
    opacity: INSPECTION_PARENT_OPACITY,
    renderOrder: 9999,
  ...
  let lastHover = null;
  let hoverExcludedLinkNames = new Set();
  let selectedMeshes = [];
  ...
  function clearSelection() {
    // Child-body highlighting belongs to the open component detail view.
    // Clearing a normal 3D selection (for example by clicking empty space) must
    // not remove it; ComponentsPanel clears it explicitly when the detail closes.
    selectedMeshes = [];
    selectedLink = null;
  ...
  function showLinksWithOverlay(overlay, linkNames = []) {
    ...
    for (const value of names) {
      ...
      shown.add(link);
      overlay.showLink(link);
    }
    return shown.size;
  }
  function highlightLinks(linkNames = []) {
    childHighlight.clear();
    const count = showLinksWithOverlay(childHighlight, linkNames);
    requestRender();
    return count;
  }
  function highlightInspection(parentLinkName = '', childLinkNames = []) {
    parentHighlight.clear();
    childHighlight.clear();
    const parentCount = showLinksWithOverlay(parentHighlight, parentLinkName ? [parentLinkName] : []);
    const childCount = showLinksWithOverlay(childHighlight, childLinkNames);
    requestRender();
    return { parentCount, childCount };
  }
  function setHoverExcludedLinks(linkNames = []) {
    hoverExcludedLinkNames = new Set((Array.isArray(linkNames) ? linkNames : [linkNames]).map(String).filter(Boolean));
    hover.clear();
    lastHover = null;
    requestRender();
    return hoverExcludedLinkNames.size;
  }
  function clearRelatedHighlights() {
    parentHighlight.clear();
    childHighlight.clear();
    hoverExcludedLinkNames.clear();
    requestRender();
  }
  ...
  function processHoverMove(ev) {
    ...
    const pickedLinkName = String(pick?.linkName || pick?.link?.userData?.__linkName || pick?.link?.name || '');
    const hoverAllowed = !pickedLinkName || !hoverExcludedLinkNames.has(pickedLinkName);
    if (key !== lastHover || !hoverAllowed) {
      hover.clear();
      lastHover = hoverAllowed ? key : null;
      if (pick?.link && hoverAllowed) hover.showLink(pick.link);
      requestRender();
    }
  ...
  return {
    ...
    highlightLinks,
    highlightInspection,
    setHoverExcludedLinks,
    clearRelatedHighlights,
    refreshSelectionMarker,
    destroy() {
      destroyed = true;
      if (dragCommitRAF) { cancelAnimationFrame(dragCommitRAF); dragCommitRAF = 0; }
      hover.dispose?.(); parentHighlight.dispose?.(); childHighlight.dispose?.(); if (selectionHelper) scene.remove(selectionHelper);
