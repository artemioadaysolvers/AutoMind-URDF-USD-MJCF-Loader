...
// MJCF BUILD234 interaction: pooled translucent hover overlays, indexed picking and nonblocking drag commits.
/* global THREE */
...
function buildHoverOverlay({ color = HOVER_COLOR, opacity = HOVER_OPACITY, getSectionPlane = null, getMeshesInLink = collectMeshesInLink } = {}) {
  // BUILD234: preserve the translucent duplicate-geometry highlight without the
  // BUILD232 overdraw/allocation cost. Overlay meshes share the source geometry,
  // all use one material, are pooled across hover changes, and keep depth testing
  // enabled so hidden/overlapping CAD surfaces are rejected by the depth buffer.
  const active = [];
  const pool = [];
  ...
  sharedMaterial.userData = {
    ...(sharedMaterial.userData || {}),
    __automindHoverOverlayMaterial: true
  };

  function syncClipping() {
    const plane = typeof getSectionPlane === 'function' ? getSectionPlane() : null;
    const enabled = !!plane;
    if (enabled) sharedMaterial.clippingPlanes = [plane];
    else sharedMaterial.clippingPlanes = null;
    // Shader recompilation is only required when clipping changes on/off, not
    // while the same live Plane object moves.
    if (clippingEnabled !== enabled) {
  ...
  function acquireOverlay() {
    const ov = pool.pop() || new THREE.Mesh(undefined, sharedMaterial);
    ov.material = sharedMaterial;
    ov.visible = true;
    ov.renderOrder = 9999;
    ...
    // Absolute safety net: overlays can never participate in any raycaster,
    // including external tools that recursively raycast the full scene.
    ov.raycast = () => {};
    return ov;
  }
  ...
  function releaseOverlay(ov) {
    if (!ov) return;
    try { ov.parent?.remove(ov); } catch (_) {}
    ov.geometry = null;
    ov.visible = false;
    ov.userData.__hoverSourceMesh = null;
    pool.push(ov);
  }

  function clear() {
    while (active.length) releaseOverlay(active.pop());
  }

  function showMesh(mesh) {
    const plane = syncClipping();
    if (!meshPickableNow(mesh, plane) || !mesh?.geometry) return;
    const ov = acquireOverlay();
    // No clone: source and overlay reference the exact same BufferGeometry.
    ov.geometry = mesh.geometry;
    ov.frustumCulled = mesh.frustumCulled !== false;
    ov.userData.__hoverSourceMesh = mesh;
    // Parenting to the source keeps the duplicate skin perfectly synchronized
    // during explode, joint motion and camera interaction with no matrix copies.
    mesh.add(ov);
    active.push(ov);
  }
  ...
  function showLink(link) {
    const plane = syncClipping();
    for (const mesh of getMeshesInLink(link)) {
      if (!meshPickableNow(mesh, plane)) continue;
      const ov = acquireOverlay();
      ov.geometry = mesh.geometry;
      ov.frustumCulled = mesh.frustumCulled !== false;
      ov.userData.__hoverSourceMesh = mesh;
  ...
  return { clear, showMesh, showLink, dispose };
}
...
export function attachInteraction({ scene, camera, renderer, controls, robot, selectMode = 'link', getSectionPlane = null, onSelectLink = null, onKinematicCommit = null, invalidate = null }) {
  ...
  return {
    ...
    destroy() {
      destroyed = true;
      if (dragCommitRAF) { cancelAnimationFrame(dragCommitRAF); dragCommitRAF = 0; }
      hover.dispose?.(); if (selectionHelper) scene.remove(selectionHelper);
