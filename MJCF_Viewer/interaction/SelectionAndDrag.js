...
// MJCF BUILD233 interaction: nonblocking drag commits and frame-sliced closed-chain settling.
/* global THREE */
...
function buildHoverOverlay({ color = HOVER_COLOR, getSectionPlane = null, getMeshesInLink = collectMeshesInLink } = {}) {
  // BUILD233: hover must not duplicate CAD geometry. The previous translucent
  // overlay rendered every hovered triangle a second time with depthTest=false.
  // On assembled mechanisms with many overlapping parts this created extreme
  // fragment overdraw; exploding the model merely hid the symptom by separating
  // the surfaces. Reuse cached tinted material clones instead, keeping the exact
  // same geometry and draw-call count as the normal model.
  const touched = [];
  const tintedByOriginal = new WeakMap();
  const tint = new THREE.Color(color);

  function materialRefsEqual(a, b) {
    if (a === b) return true;
  ...
  function tintedMaterial(original) {
    if (!original) return original;
    let cached = tintedByOriginal.get(original);
    if (cached) return cached;

    const mat = original.clone?.() || original;
    if (mat !== original) {
      // Preserve texture, clipping, transparency and render mode. Only alter the
      // inexpensive color/emissive uniforms; no transparent overlay pass is added.
      ...
      mat.userData = { ...(mat.userData || {}), __automindHoverMaterial: true };
      mat.needsUpdate = false;
    }
    tintedByOriginal.set(original, mat);
    return mat;
  }

  function clear() {
    while (touched.length) {
      const rec = touched.pop();
      if (!rec?.mesh) continue;
      // Do not overwrite a newer render-mode/section material transaction.
      if (materialRefsEqual(rec.mesh.material, rec.highlighted)) rec.mesh.material = rec.original;
    }
  }
  ...
  function showMesh(mesh) {
    ...
    const highlighted = Array.isArray(original)
      ? original.map(tintedMaterial)
      : tintedMaterial(original);
    mesh.material = highlighted;
    touched.push({ mesh, original, highlighted });
  }
...
export function attachInteraction({ scene, camera, renderer, controls, robot, selectMode = 'link', getSectionPlane = null, onSelectLink = null, onKinematicCommit = null, invalidate = null }) {
  ...
  function pickInfoFromPointer(ev) {
    ...
    // BUILD233 broad phase: sort meshes by the ray entry distance into their
