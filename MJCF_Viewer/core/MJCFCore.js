...
import { variantsFor, basenameNoQuery, dataURLFor } from './AssetDB.js';
...
class LazyMJCFAssetSource {
  constructor() {
    ...
    this.zip = null;
    this.stats = { zipEntries: 0, textInflations: 0, textureInflations: 0, indexedPaths: 0 };
  }
  _indexPath(actual) {
    const key = cleanPath(actual);
    if (!key) return;
    for (const variant of variantsFor(key)) {
      if (!this.pathIndex.has(variant)) this.pathIndex.set(variant, key);
    }
...
async function addGeom(node, content, model, assets, assetSource, textureCache, linkName) {
  ...
  if (type === 'mesh') {
    ...
    model.userData.__automindGeometryCompaction = model.userData.__automindGeometryCompaction || { meshes: 0, verticesBefore: 0, verticesAfter: 0 };
...
export async function loadMJCFModel(opts = {}) {
  ...
  try {
    ...
    model.userData.__automindBuild = 'MJCF BUILD231 — lazy ZIP, direct textures, indexed OBJ geometry';
    ...
    try {
      ...
      console.info('[AutoMind MJCF] Carga optimizada:', report);
    } catch (_) {}
    textureCache.clear();
    await sleep(0);
    return model;
