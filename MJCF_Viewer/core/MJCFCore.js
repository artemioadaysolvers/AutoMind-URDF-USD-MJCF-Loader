...
// AutoMind BUILD225 MJCF loader — adaptive frame-budgeted kinematics.
...
class MJCFModel extends THREE.Group {
  constructor(name = 'AutoMindMJCF') {
    ...
    this.kinematicPerformance = {
      ...
      dragMinIterations: 2,
      dragMaxIterations: 7,
      dragInitialIterations: 4,
      dragMinVariables: 4,
      dragMaxVariables: 12,
      dragInitialVariables: 8,
      dragFallbackAttempts: 1,
      settleIterations: 32,
      settleVariables: 24
    };
  ...
  _applyConstrainedJointDrag(joint, targetValue) {
    ...
    // The former eight-step binary search could execute nine complete DLS solves
    // for one mouse event. Permit at most one midpoint rescue, and only when the
    // first solve used less than half of the interactive frame budget.
    const perf = this.kinematicPerformance || {};
    const budget = Number(perf.dragBudgetMs) || 7.5;
    const mayFallback = (perf.dragFallbackAttempts || 0) > 0 &&
      Number(this.lastLoopSolve?.elapsedMs || Infinity) < budget * 0.5 &&
      Math.abs(targetValue - startValue) > 1e-8;
    if (mayFallback) {
      const mid = this._attemptConstrainedDragValue(joint, startValue + (targetValue - startValue) * 0.5, baseSnapshot);
      if (mid.residual <= limit) {
        this._restoreMovableJointValues(mid.snapshot);
        this.applyPose(joint, { solve: false, publish: true, full: true });
        return true;
      }
    }
...
export async function loadMJCFModel(opts = {}) {
  ...
  model.userData.__automindBuild = 'MJCF BUILD224 — frame-budgeted kinematics, coalesced drag and single-pass presentation';
