...
async function ensureObjLoader() {
  ...
  if (!THREE.OBJLoader) throw new Error('OBJLoader unavailable');
}
...
function stampMaterialTextureState(mat) {
  ...
  // IMPORTANT: never put a live THREE.Texture inside material.userData.
  // THREE.Material.clone() deep-copies userData through JSON, which turns the
  // Texture into a plain object. Restoring that object into material.map later
  // crashes WebGLUniforms at uvTransform.copy(texture.matrix).
  try { delete mat.userData.__automindTextureMap; } catch (_) {}
  ...
  mat.userData.__automindTextureLocked = !!mat.map?.isTexture;
  return mat;
}
...
async function makeMaterial(def, assetSource, textureCache, geomNode = null) {
  ...
  if (map) {
    ...
    const flatTextureColor = map.userData?.__automindFlatTextureColor || null;
    const intentionalFlatBlack = Array.isArray(flatTextureColor) &&
      Number(flatTextureColor[3] ?? 1) > 0.05 &&
      Number(flatTextureColor[0] || 0) < 0.025 &&
      Number(flatTextureColor[1] || 0) < 0.025 &&
      Number(flatTextureColor[2] || 0) < 0.025;

    // A uniformly black CAD swatch is an authored black material, not a failed
    // texture upload. Only use the emergency visible colour for suspicious dark
    // bitmaps that are not a decoded flat-black texture.
    const useEmergencyFallback = textureLooksBlack && !intentionalFlatBlack &&
      (rgbaLuma > 0.035 || forceVisible);
