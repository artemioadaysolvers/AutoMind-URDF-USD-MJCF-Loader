...
// AutoMind BUILD227 MJCF loader — synchronized passive-ball closed-chain kinematics.
...
class MJCFModel extends THREE.Group {
  constructor(name = 'AutoMindMJCF') {
    ...
    this.kinematicPerformance = {
      // Closed CAD mechanisms need every passive coordinate to move in the same
      // visual frame.  BUILD224's very small interactive budget could publish an
      // under-solved pose and then visibly jump during the release settle.
      dragBudgetMs: 14,
      dragMinIterations: 4,
      dragMaxIterations: 12,
      dragInitialIterations: 7,
      dragMinVariables: 8,
      dragMaxVariables: 32,
      dragInitialVariables: 18,
      dragFallbackAttempts: 4,
      settleIterations: 48,
      settleVariables: 64
    };
  ...
  _passiveLoopSolverJoints(changedJoint = null) {
    ...
    const limit = this.isDraggingJoint
      ? Math.max(perf.dragMinVariables || 4, Math.min(perf.dragMaxVariables || 12, this._dragAdaptiveVariables || perf.dragInitialVariables || 8))
      : (perf.settleVariables || 24);
  ...
  _solveLoopClosures(changedJoint = null) {
    ...
    if (!loops.length || this.isSolvingLoops) return this.lastLoopSolve;
    ...
    const maxConstraints = dragging ? 256 : 2400;
    const tolerance = Math.max(this._modelScale() * 1.25e-4, dragging ? 3.5e-5 : 8e-6);
    const started = performance.now();
    const budgetMs = dragging ? Math.max(2, Number(perf.dragBudgetMs) || 7.5) : Infinity;

    if (!variables.length) {
      const residual = this._loopErrorNorm(this._loopResidual(maxConstraints));
      const elapsedMs = performance.now() - started;
      this.lastLoopSolve = { residual, vars: 0, iterations: 0, constraints: loops.length, converged: residual <= tolerance, elapsedMs };
    ...
    try {
      const maxIter = dragging
        ? Math.max(perf.dragMinIterations || 2, Math.min(perf.dragMaxIterations || 7, this._dragAdaptiveIterations || perf.dragInitialIterations || 4))
        : (perf.settleIterations || 32);
      let lambda = dragging ? 3e-5 : 3e-7;
      let residual = Infinity;
      let converged = false;
      const reference = variables.map(j => j.value || 0);

      for (let iter = 0; iter < maxIter; iter++) {
        if (dragging && iter > 0 && performance.now() - started >= budgetMs) break;
        ...
        for (let c = 0; c < n; c++) {
          ...
          if (dragging) {
            // Forward differences halve expensive kinematic evaluations during
            // pointer interaction. The final settle restores central differences.
        ...
        for (let i = 0; i < n; i++) {
          ...
          const poseReg = dragging ? 1.2e-6 : 1e-7;
        ...
        const maxStep = dragging ? 0.11 : 0.28;
        ...
        let accepted = false;
        const lineSearch = dragging ? [1, 0.5, 0.25] : [1, 0.65, 0.4, 0.25, 0.125, 0.0625];
        for (const alpha of lineSearch) {
          ...
          if (Number.isFinite(trial) && trial + Math.max(1e-12, residual * 1e-6) < residual) {
            residual = trial;
            lambda = Math.max(lambda * 0.35, 1e-10);
            accepted = true;
            break;
          }
        }
      ...
      if (dragging) {
        const minI = perf.dragMinIterations || 2, maxI = perf.dragMaxIterations || 7;
        const minV = perf.dragMinVariables || 4, maxV = perf.dragMaxVariables || 12;
        if (elapsedMs > budgetMs * 1.12) {
          this._dragAdaptiveIterations = Math.max(minI, (this._dragAdaptiveIterations || 4) - 1);
          this._dragAdaptiveVariables = Math.max(minV, (this._dragAdaptiveVariables || 8) - 1);
        } else if (elapsedMs < budgetMs * 0.58) {
          this._dragAdaptiveIterations = Math.min(maxI, (this._dragAdaptiveIterations || 4) + 1);
          this._dragAdaptiveVariables = Math.min(maxV, (this._dragAdaptiveVariables || 8) + 1);
        }
      }
  ...
  _attemptConstrainedDragValue(joint, value, baseSnapshot) {
    ...
    this.applyPose(joint, { publish: false, full: false, solve: true });
  ...
  _applyConstrainedJointDrag(joint, targetValue) {
    ...
    // Never use a residual cached from a different pointer frame.  A stale
    // low value could accept an open mechanism; a stale high value could reject a
    // valid movement and visibly reset the selected body.
    const baseResidual = this._loopErrorNorm(this._loopResidual());
    ...
    if (!this._activeSolverLoops().length || direct.residual <= limit) {
      ...
      this.applyPose(joint, { solve: false, publish: true, full: true });
      return true;
    }

    // Continue toward the requested coordinate with progressively smaller steps.
    // Returning immediately to the previous snapshot looked like a reset and left
    // the user waiting for a later click/release settle.  The first feasible
    // fraction is committed in the same animation frame instead.
    const perf = this.kinematicPerformance || {};
    const budget = Number(perf.dragBudgetMs) || 14;
    const attempts = Math.max(0, Math.min(4, Number(perf.dragFallbackAttempts) || 0));
    const fractions = [0.75, 0.5, 0.25, 0.125];
    const fallbackStarted = performance.now();
    for (let i = 0; i < attempts; i++) {
      if (i > 0 && performance.now() - fallbackStarted > budget * 1.6) break;
      const fraction = fractions[i];
      const candidate = this._attemptConstrainedDragValue(
        joint,
        startValue + (targetValue - startValue) * fraction,
        baseSnapshot
      );
      if (candidate.residual <= limit) {
        this._restoreMovableJointValues(candidate.snapshot);
        this.applyPose(joint, { solve: false, publish: true, full: true });
        return true;
      }
    }
  ...
  applyPose(changedJoint = this._lastCommandJoint || null, options = {}) {
    ...
    try {
      this._applyCouplings();
      this._refreshLinkMatrices({ full: false, publish: false });
      if (solve) this._solveLoopClosures(changedJoint);
      this._applyCouplings();
  ...
  beginInteractiveDrag(joint = null) {
    ...
    const perf = this.kinematicPerformance || {};
    this._dragAdaptiveIterations = perf.dragInitialIterations || 4;
    this._dragAdaptiveVariables = perf.dragInitialVariables || 8;
    this._lastStableSnapshot = this._snapshotMovableJointValues();
    this.userData.__automindInteractionMode = 'kinematic-drag';
  }
  ...
  endInteractiveDrag(joint = null) {
    ...
    this.activeJointForDrag = null;
    this.isDraggingJoint = false;
    this.userData.__automindInteractionMode = 'settling';
    this.applyPose(joint || this._lastCommandJoint || null, { solve: true, publish: true, full: true });
    this.pinnedSolverJointName = '';
...
export async function loadMJCFModel(opts = {}) {
  ...
  model.userData.__automindBuild = 'MJCF BUILD227 — passive ball DOFs, synchronized closed chains and lossless drag finalization';
