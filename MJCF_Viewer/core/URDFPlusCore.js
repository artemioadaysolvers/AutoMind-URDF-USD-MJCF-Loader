...
import { installOBJLoaderLite } from './OBJLoaderLite.js';
...
const AUTOMIND_TEXTURE_PIPELINE = {
  ...
  // once.
  // BUILD165: 3x faster than BUILD164 while keeping deterministic sequencing.
  DAE_INLINE_SETTLE_MS: 1,
  DAE_PARSE_SETTLE_MS: 2,
  DAE_TEXTURE_TIMEOUT_MS: 389,
  DAE_POST_TEXTURE_SETTLE_MS: 4,
  BETWEEN_DAE_LOADS_MS: 2,
};
...
function automindSolidifyCadMaterial(mat) {
  ...
  try {
    ...
    if (mat.map && mat.color) {
      // BUILD163: preserve DAE diffuse tint. Some DAE textures are grayscale/detail
      // maps and the real CAD color is in material.diffuse. Forcing white makes
      // red/black parts look gray, like fog.
...
// BUILD164: make the main viewport use the same color philosophy as the
// thumbnail renderer. The thumbnails looked correct because they used an unlit
// MeshBasicMaterial with the original DAE diffuse tint multiplied by the map.
// The combined viewport was still using lit CAD/Collada materials, so on the
// white Colab canvas the result looked gray/foggy. This conversion is applied
// only to visible CAD mesh materials; render modes still control opacity/wireframe.
function textureColorSpaceSRGB(tex) {
  if (!tex) return tex;
...
function copyTexSlot(dst, src, slot) {
  try {
    if (src && src[slot] && !textureHasMissingFallbackSource(src[slot])) {
      dst[slot] = textureColorSpaceSRGB(src[slot]);
    }
  } catch (_) {}
}
function cloneMaterialForViewportNoFog(src, fallbackColor = 0xdfe8ea) {
  if (!src || typeof THREE === 'undefined') {
    const m0 = new THREE.MeshBasicMaterial({ color: fallbackColor, side: THREE.DoubleSide });
    try { m0.toneMapped = false; } catch (_) {}
    return m0;
  }
  const hasMap = src.map && !textureHasMissingFallbackSource(src.map);
  const color = (src.color && typeof src.color.clone === 'function') ? src.color.clone() : new THREE.Color(fallbackColor);
  const alphaTest = Number.isFinite(src.alphaTest) ? src.alphaTest : 0;
  const hasAlpha = !!src.alphaMap || alphaTest > 0.001 || (Number.isFinite(src.opacity) && src.opacity < 0.999);
  const m = new THREE.MeshBasicMaterial({
    color,
    map: hasMap ? textureColorSpaceSRGB(src.map) : null,
    side: THREE.DoubleSide,
    transparent: !!src.alphaMap || alphaTest > 0.001,
    opacity: 1,
    alphaTest,
    depthWrite: true,
    depthTest: true,
  ...
  // MeshBasic supports these slots and they are useful for labels/decals/alpha.
  copyTexSlot(m, src, 'alphaMap');
  copyTexSlot(m, src, 'aoMap');
  copyTexSlot(m, src, 'lightMap');
  copyTexSlot(m, src, 'envMap');
  try {
    m.name = src.name || m.name || '';
    m.userData = { ...(src.userData || {}), __automindViewportNoFog: true, __automindOriginalMaterialType: src.type || src.constructor?.name || '' };
    m.toneMapped = false;
    m.blending = THREE.NormalBlending;
    m.premultipliedAlpha = false;
    if (hasAlpha && Number.isFinite(src.opacity) && src.opacity < 0.999) {
      // Preserve real alpha only when the source actually intended it. Bad DAE
      // transparency is removed later by Solid mode unless alphaMap/alphaTest exists.
      m.opacity = Math.max(0, Math.min(1, src.opacity));
    }
    m.needsUpdate = true;
  } catch (_) {}
  return m;
}
function applyViewportNoFogMaterials(obj) {
  obj?.traverse?.((o) => {
    if (!o?.isMesh || !o.material || o.userData?.__isHoverOverlay) return;
    if (o.userData?.__automindViewportNoFogApplied) return;
    const fallback = 0xdfe8ea;
    try {
      if (Array.isArray(o.material)) o.material = o.material.map(m => cloneMaterialForViewportNoFog(m, fallback));
      else o.material = cloneMaterialForViewportNoFog(o.material, fallback);
      o.userData.__automindViewportNoFogApplied = true;
      o.castShadow = true;
      o.receiveShadow = true;
    } catch (_) {}
  });
...
async function waitForObjectTexturesReady(obj, timeoutMs = AUTOMIND_TEXTURE_PIPELINE.DAE_TEXTURE_TIMEOUT_MS) {
  ...
  const per = Math.max(30, Math.floor(timeoutMs / Math.max(1, Math.min(4, maps.length))));
...
async function loadMeshCandidate(candidate, resolver, fallbackMaterial) {
  ...
  if (ext === '.dae') {
    ...
    await waitForObjectTexturesReady(obj, Math.max(120, AUTOMIND_TEXTURE_PIPELINE.DAE_TEXTURE_TIMEOUT_MS / 2));
    solidifyCadMaterials(obj);
    applyViewportNoFogMaterials(obj);
  ...
  if (ext === '.stl') {
    ...
    solidifyCadMaterials(g);
    applyViewportNoFogMaterials(g);
    return g;
  }
  if (ext === '.obj') {
